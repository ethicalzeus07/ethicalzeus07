<!-- ========================= THEME =========================
Primary: #FFA500  |  Accent: #FF6F00  |  BG: #0D0D0D
=========================================================== -->

<!-- TOP WAVE BANNER -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=220&color=0:FF6F00,100:FFA500&text=Pravar%20Chauhan&fontColor=0D0D0D&fontSize=48&fontAlignY=38&animation=twinkling&desc=Accessible%20AI%20%E2%80%A2%20Cloud%20Systems%20%E2%80%A2%20FastAPI&descAlignY=60"/>
</p>

<!-- Custom Main Title -->
<h1 align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Orbitron&weight=700&size=32&pause=1000&color=FFA500&center=true&vCenter=true&width=650&lines=Pravar+Chauhan" alt="Pravar Chauhan"/>
</h1>

<h3 align="center">Software Engineer · Accessible AI · Cloud & Serverless · FastAPI · Docker · AWS</h3>

<!-- Typing Animation -->
<p align="center">
  <a href="https://github.com/ethicalzeus07">
    <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&pause=1000&color=FFA500&center=true&vCenter=true&width=820&lines=Building+accessible+AI+systems+for+real+users;Cloud+%2B+Backend+%2B+LLMs+%2B+RAG;FastAPI+%7C+AWS+%7C+Docker+%7C+Terraform;Always+learning+%E2%80%94+always+shipping"/>
  </a>
</p>

<!-- Badges -->
<p align="center">
  <a href="mailto:chauhanpravar7@gmail.com"><img src="https://img.shields.io/badge/Email-0D0D0D?style=for-the-badge&logo=gmail&logoColor=white&labelColor=FF6F00"/></a>
  <a href="https://www.linkedin.com/in/pravar-chauhan-83845930a/"><img src="https://img.shields.io/badge/LinkedIn-0D0D0D?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=FF6F00"/></a>
  <a href="https://github.com/ethicalzeus07"><img src="https://img.shields.io/badge/GitHub-0D0D0D?style=for-the-badge&logo=github&logoColor=white&labelColor=FF6F00"/></a>
  <a href="https://d3tx6hx7gzmh0g.cloudfront.net/"><img src="https://img.shields.io/badge/Portfolio-0D0D0D?style=for-the-badge&logo=vercel&logoColor=white&labelColor=FF6F00"/></a>
  <img src="https://komarev.com/ghpvc/?username=ethicalzeus07&style=for-the-badge&color=FFA500&label=Profile%20Views" alt="views"/>
</p>

---

## ⚙️ Snapshot
- 🧩 Building **accessible AI** tools (voice + LLM) so blind/low-vision learners can use interactive STEM simulations
- ☁️ Shipping **cloud-native systems** (serverless + containers) with clean APIs, observability, and reliability-first design
- 🏆 Hackathon winner: **Asclepius-HI (VillageHacks 2025 — Supermemory Track)**

<p align="center">
  <img src="./dragon.gif" alt="Super Saiyan Goku powering up" width="240" style="border-radius:12px; box-shadow: 0 0 20px #FF6F00;"/>
</p>

---

## 🧰 Toolbox

<p align="center">
  <b>Languages</b> · <b>Backend & DevOps</b> · <b>Data & AI</b>
</p>

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,javascript,typescript,java,c,swift,bash&theme=dark&perline=20" />
</p>

<p align="center">
  <img src="https://skillicons.dev/icons?i=fastapi,docker,kubernetes,terraform,git,linux,aws&theme=dark&perline=20" />
</p>

<p align="center">
  <img src="https://skillicons.dev/icons?i=postgresql,sqlite,redis,sklearn,pytorch,flask&theme=dark&perline=20" />
</p>

<p align="center">
  <img src="./goku.gif" alt="Goku mascot" width="250" style="border-radius:12px; box-shadow:0 0 24px #FFA500;" />
</p>

---

## 📌 Featured Projects

| Project | What it is | Tech |
|--------|------------|------|
| **AI-Powered Accessible Simulation Chrome Extension** | Voice + natural-language control for interactive STEM simulations to support **blind/low-vision learners** | Chrome MV3, TypeScript, AWS Lambda, FastAPI/Node, LLM APIs |
| [**Inspark A11y Assistant**](https://github.com/ethicalzeus07/inspark-a11y) | Automates WCAG checks (axe-core) and returns actionable remediation guidance | FastAPI, JS, axe-core |
| [**Data Dash (AWS)**](https://github.com/ethicalzeus07/datadash-eks) | Real-time metrics dashboard with streaming events and cloud deployment | FastAPI, WebSockets, Docker, AWS |
| [**Ballu / Ballsy Voice Assistant**](https://github.com/ethicalzeus07/ballsy-voice-assistant) | Local voice assistant pipeline + command execution foundation | Python, WebSockets, SQLite, Docker |
| **Asclepius-HI (Hackathon Winner)** | Memory-augmented healthcare copilot concept: patient triage + clinician summaries | React, FastAPI, RAG/Vector DB, Postgres |

> Note: Some work is under NDA/academic sponsorship; public repos reflect what I’m able to share.

---

## 💼 Experience
**ASU Center for Education Through Exploration (ETX)**  
*Platform QA & Support Engineer (Student)*  

- QA/testing + support workflows for online course platform; focus on reliability, clarity, and accessibility.

---

## 🎓 Education
**New York University (NYU) — Tandon School of Engineering**  
M.S. Computer Science · Incoming Fall 2026  

**Arizona State University (ASU)**  
B.S. Computer Science · Minor in Business  

**Certifications**  
AWS Certified (Cloud Practitioner)

---

## 📊 Activity Graph
<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=ethicalzeus07&theme=high-contrast&hide_border=true&area=true&bg_color=0D0D0D&title_color=FFA500&color=FF6F00&line=FFA500&point=FF6F00" />
</p>

## 🐍 Contribution Snake
<p align="center">
  <img src="https://raw.githubusercontent.com/ethicalzeus07/ethicalzeus07/output/snake.svg"/>
</p>

<p align="center">
  <img src="./scene.gif" alt="Pixel Sunset Scene" width="700" style="border-radius:12px; box-shadow: 0 0 25px #FF6F00;"/>
</p>

<!-- FOOTER WAVE -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=120&color=0:FFA500,100:FF6F00&section=footer"/>
</p>
